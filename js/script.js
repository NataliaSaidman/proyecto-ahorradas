...
const find = (array, propiedad, valor) => {
  return array.find((obj) => obj[propiedad] == valor);
};

const setItemLocalStorage = (storage) =>
...
const generateId = () => {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    arrayOne.push(
      idStringLetters[randomIdLetters] + idStringNumber[randomIdNumber]
    );
  }
  for (let i = 0; i < 3; i++) {
    const randomIdLetters = Math.floor(Math.random() * idStringLetters.length);
...
const dataCategoriesLocalStorage = () => {
  return JSON.parse(localStorage.getItem("storage")).categories;
};
const dataOperationsLocalStorage = () => {
  return JSON.parse(localStorage.getItem("storage")).operations;
};
const dataLocalStorage = (key) => {
...
const categoryAddLocalStorage = () => {
  const name =
    $addCategoriesInput.value.charAt(0).toUpperCase() +
    $addCategoriesInput.value.slice(1);
  const id = generateId();
...
const categoryAlert = (input) => {
  if (input.value === "") {
    return alert(
      `Por favor ingrese el nombre de la categoría que desea agregar`
    );
...
const removeCategoryLocal = (id) => {
  ...
  const storage = {
    ...localData,
    categories: filter(dataLocalStorage("categories"), "id", id),
    operations: operationRemoveByFilterCategory(id),
  };
  setItemLocalStorage(storage);
};
  ...
  if (arrOperationLocal.length !== 0) {
    for (const { category } of arrOperationLocal) {
      if (!arrCategoriesLocal.includes(id) && category === id) {
        arrOperationLocal = dataLocalStorage("operations").filter(
...
const removeCategory = (id) => {
  ...
  if (filteredByRemovedCategory.length !== 0)
    addNewOperation(filteredByRemovedCategory);
};
...
const deleteCategory = () => {
  return confirm(
    `Está por eliminar una categoria. Al eliminarla, se borrarán todas las operaciones ingresadas de dicha categoria ¿Desea continuar?`
  );
};
  ...
  for (const operation of operations) {
    if (chosenOperation.id === operation.id) {
      ...
      operation.description =
        $("#editDescription").value.charAt(0).toUpperCase() +
        $("#editDescription").value.slice(1);
...
const removeOperationLocal = (id) => {
  ...
  const storage = {
    ...localData,
    operations: filter(dataLocalStorage("operations"), "id", id),
  };
  setItemLocalStorage(storage);
};
    ...
    localOperations.map(({ id, description, amount, type, category, date }) => {
      ...
      tr.innerHTML += `
            ...
            <th class="">
                <div class="w-24 lg:min-w-[9rem] ml-2 text-start truncate h-7">
                    <span class="bg-[#f8b6ce] px-2 py-1 rounded-md text-[#ab062d] text-xs">${nameCategory(
                      category
                    )}</span>
                </div>
            </th>
      ...
      trResponsive.innerHTML += `
            ...
            <th class="sm:ml-5 ml-4 mt-1 truncate">
                <div>
                    <span class="bg-[#f8b6ce] px-2 py-1 rounded-md text-[#ab062d] text-sm">${nameCategory(
                      category
                    )}</span>
                </div>
    ...
    for (const btn of btnRemove) {
      ...
      btn.addEventListener("click", (e) => {
        ...
        if (deleteOperation()) {
          ...
          addNewOperation(
            filter(dataLocalStorage("operations"), "id", operationId)
          );
          operationsEmptyOrNot();
          filterFunction();
          enoughOperations();
...
const filterType = (array) => {
  if (array.length !== 0) {
    for (const operation of array) {
      ...
      if (
        filterOperationByProp(array, "type", $typeFilter.value).length === 0
      ) {
...
const categoryFilter = (array) => {
  if (array.length !== 0) {
    for (const operation of array) {
      ...
      if (
        filterOperationByProp(array, "category", $categoryFilter.value)
          .length === 0
      ) {
        emptyOperationsAndBalance();
        return (array = []);
...
const orderBy = (array) => {
  ...
  if ($orderBy.value === "1")
    return array.sort((a, b) => changeDate(b) - changeDate(a));
  if ($orderBy.value === "2")
    return array.sort((a, b) => changeDate(a) - changeDate(b));
...
const filterByCategory = (category) => {
  return dataLocalStorage("operations").filter(
    (operation) => operation.category === category
  );
};
...
const summaryReports = () => {
  ...
  $categoryMaxProfit.innerHTML = `
        ...
        <div class="sm:w-1/3 sm:text-end">
            <span class="bg-[#f8b6ce] px-2 py-1 rounded-md text-[#ab062d] text-xs font-bold"">${nameCategory(
              maxEarnings.maxCategory
            )}</span>
        </div>
        <div class="sm:w-1/3 sm:text-end font-semibold">
            <span class="text-green-500 font-semibold">+$${
              maxEarnings.maxAmount
            }</span>
        </div>
    `;
  ...
  const { maxMonthAmount, maxMonth, minMonthAmount, minMonth } =
    monthMaxAndMin();
  ...
  for (const obj of Object.keys(categoriesTotalBalance)) {
    ...
    $tableCategoriesReports.innerHTML += `
            <tr class="w:full flex justify-between">
                ...
                <th class="font-medium">
                    <div class="w-20 lg:min-w-[9rem] mr-1 font-medium text-end">
                        <p class="sm:text-base text-sm">${symbolBalance(
                          balance
                        )}</p>
                    </div>
                </th>
...
const restartWebAlert = () => {
  return confirm(
    `Está por eliminar todas las operaciones ingresadas hasta el momento ¿Desea continuar?`
  );
};
...
const arrOfTabs = [
  $linkNavbar_balance,
  $linkNavbar_categories,
  $linkNavbar_reports,
  $btnFormNewOperation,
];

const arrOfTabsBurger = [
  $linkNavBarBurger_balance,
  $linkNavbarBurger_categories,
  $linkNavbarBurger_reports,
  $btnFormNewOperation,
];
